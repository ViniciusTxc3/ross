...
def _(rs, steel):
    ...
    bearings = [rs.BearingElement(n=0, kxx=1000000.0, kyy=800000.0, cxx=100, cyy=100), rs.BearingElement(n=6, kxx=1000000.0, kyy=800000.0, cxx=100, cyy=100)]
...
@app.cell
def _(Q_, np, rotor):
    _n1 = 2  # out-of-balancing is positioned at the left disk
    m1 = 0.001  # amount of out-of-balancing expressed in kg*m
    _p1 = 0  # ou-of-balancing mass phase position
    _speed = Q_(np.linspace(10, 4500, 1950), 'RPM')
    results_case1 = rotor.run_unbalance_response([_n1], [m1], [_p1], _speed)
...
def _(Q_, results_case1):
    _probe1 = (2, 0)
    _probe2 = (2, Q_(90, 'deg'))
    fig_1 = results_case1.plot_magnitude(probe=[_probe1, _probe2], probe_units='degrees', frequency_units='RPM', amplitude_units='µm pkpk')
...
def _(Q_, results_case1):
    _probe1 = (4, 0)
    _probe2 = (4, Q_(90, 'deg'))
    fig_2 = results_case1.plot_magnitude(probe=[_probe1, _probe2], probe_units='degrees', frequency_units='RPM', amplitude_units='µm pkpk')
    return (fig_2,)
...
@app.cell
def _(m1, np, rotor):
    _speed = 751 * (2 * np.pi / 60)  # Q_(496, "RPM")
...
def _(go, np, response3):
    ...
    _orb4.update_layout(title='Response at node 2 and 4 at 751 RPM')
...
def _(go, np, response3_1):
    ...
    _orb4.update_yaxes(range=[-1.2 * _max_amp, 1.2 * _max_amp])
    _orb4.update_traces(x=_x_new4, y=_y_new4, name='orb4')
    _orb4.update_layout(title='Response at node 2 and 4 at 800 RPM')
    _orb4.add_trace(_starting_point4)
    _orb4.add_trace(_starting_point2)
    _orb4.add_trace(_orb2_curve)
    _orb4
...
def _(go, np, response3_2):
    ...
    _orb4.update_yaxes(range=[-1.2 * _max_amp, 1.2 * _max_amp])
    _orb4.update_traces(x=_x_new4, y=_y_new4, name='orb4')
    _orb4.update_layout(title='Response at node 2 and 4 at 2320 RPM')
    _orb4.add_trace(_starting_point4)
    _orb4.add_trace(_starting_point2)
    _orb4.add_trace(_orb2_curve)
    _orb4
...
def _(
    ...
    _cutoff = -int(time_samples_3 * 0.0018)
    ...
    _cutoff = -int(time_samples_3 * 0.0017)
    ...
    _cutoff = -int(time_samples_3 * 0.00058)
...
def _(Q_, np, rotor):
    ...
    m1_1 = 0.001
    _p1 = 0
    _speed = Q_(np.linspace(0, 4500, 950), 'RPM')
    results_case1_1 = rotor.run_unbalance_response([_n1], [m1_1], [_p1], _speed)
    _probe1 = (2, 'major')
    _probe2 = (4, 'major')
    fig_4 = results_case1_1.plot_magnitude(probe=[_probe1, _probe2], probe_units='degrees', frequency_units='RPM', amplitude_units='µm pkpk')
    fig_4.update_layout(title='Major Axis Amplitude (µm pkpk)', yaxis=dict(title='Amplitude (µm pkpk)', type='log'))
    return
